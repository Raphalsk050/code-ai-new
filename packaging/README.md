...
self-hosted Linux runner (`code-linux`), which is the only runner the company
has - GitHub's hosted ones are not reachable from there, so the label is not
an optimisation to drop. It runs on `workflow_dispatch`, on a push to any
branch, and on `v*` tags, uploading two artifacts: `code-ai-linux-x86_64` and
