...
async def test_the_index_bar_redraws_on_screen_as_progress_arrives(tmp_path) -> None:
    """The widget has to change, not just the view model behind it."""
    ...
    async with terminal_app.run_test(size=(120, 44)) as pilot:
        widget = terminal_app.query_one("#index-progress")
        assert widget.display is False

        painted: list[str] = []
        for done in (0, 30, 60):
            await terminal_app._on_event(
                EventEnvelope.create(
                    event_type="index.progress",
                    session_id="s",
                    sequence=done,
                    payload={"phase": "indexing", "done": done, "total": 60},
                    source="index",
                )
            )
            await pilot.pause(0.05)
            assert widget.display is True
            painted.append(str(widget.content))

        assert len(set(painted)) == 3, f"the bar did not move: {painted}"
        assert "0/60" in painted[0] and "60/60" in painted[-1]

        # When the refresh ends the bar goes away rather than sitting at 100%.
        await terminal_app._on_event(
            EventEnvelope.create(
                event_type="index.refreshed",
                session_id="s",
                sequence=99,
                payload={"indexed": 60},
                source="index",
